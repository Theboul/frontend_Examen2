import { useState } from "react";
import { motion } from "framer-motion";
import FormCarreras from "../components/FormCarreras";
import TableCarreras from "../components/TableCarreras";
import Header from "../../../components/common/Header";
import Footer from "../../../components/common/Footer";

export default function GestionPage() {
    const [refresh, setRefresh] = useState(false);

    // Función para refrescar la tabla cuando se crea o edita una carrera (vía modal)
    const handleSuccess = () => setRefresh(!refresh);

    return (
        <div className="min-h-screen w-full bg-white flex flex-col">
            {/* HEADER */}
            <Header />

            {/* ANIMACIÓN PRINCIPAL */}
            <motion.main
                className="flex-grow mx-auto w-full max-w-screen-2xl px-4 sm:px-6 lg:px-8 py-6"
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, ease: "easeOut" }}
            >
                {/* TÍTULO */}
                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-[#2A3964] mb-6 border-b-4 border-[#880000] pb-2 text-center sm:text-left">
                    Gestión de Carreras
                </h1>

                {/* FORMULARIO DE CREACIÓN */}
                <motion.div
                    initial={{ opacity: 0, scale: 0.95 }}
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ duration: 0.5, delay: 0.2 }}
                >
                    {/* Se usa onSuccess para que al crear, la tabla se actualice */}
                    <FormCarreras onSuccess={handleSuccess} />
                </motion.div>

                {/* TABLA */}
                <motion.div
                    className="mt-8"
                    initial={{ opacity: 0, y: 40 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.6, delay: 0.3 }}
                >
                    <TableCarreras refresh={refresh} />
                </motion.div>
            </motion.main>

            {/* FOOTER */}
            <Footer />
        </div>
    );
}